...
const Index = () => {
  ...
  const [model3Default, setModel3Default] = useState([]);
  ...
  const handlePageChange = async () => {
    ...
    response.data.map((item) => {
      if (item.AraclarModel === "Model 3") {
        ...
        setModel3Default(model3Default => [...model3Default, item])
      }
    })
  }
  useEffect(() => {
  ...
  useEffect(() => {
    const newFiltredData = model3Default.filter((item) => {
      return item.AraclarName.toLowerCase().includes(search.toLowerCase())
    })
    setModel3(newFiltredData)
  }, [search])
  const columns = [
    {
      Header: "Araba Adı",
        ...
        return (
          <div className="btn-group">
            <div className='hstack gap-2 justify-content-end'>
              <Link to={"/admin/model-3-update/" + cellProps.row.original.AraclarID} className="fw-medium link-primary"><FeatherIcon size={12} color={'success'} icon="eye" /></Link>
                  ...
                  <div className="page-title-right">
                    <ol className="breadcrumb m-0">
                      <li className="breadcrumb-item">
                        <Link to={"/admin/model-3"} >Arabalar</Link>
                      </li>
                      <li className="breadcrumb-item active">Model-3</li>
                    </ol>
                  </div>
                </div>
              </CardBody>
                  ...
                  <div className="card-header border-0">
                    <div className="row g-4">
                      ...
                      <div className="col-sm-auto">
                        <div>
                          <Link
                            to="/admin/model-3-add/"
                            className="btn btn-success"
                          >
                            <i className="ri-add-line align-bottom me-1"></i> Model-3 Ekle
                          </Link>
                        </div>
                      </div>
