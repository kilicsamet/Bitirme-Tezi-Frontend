...
const Index = () => {
  ...
  const [modelYDefault, setModelYDefault] = useState([]);
  ...
  const handlePageChange = async () => {
    ...
    response.data.map((item) => {
      if (item.AraclarModel === "Model Y") {
        setModelY(modelY => [...modelY, item])
        setModelYDefault(modelYDefault => [...modelYDefault, item])
      }
    })
  }
  useEffect(() => {
  ...
  useEffect(() => {
    const newFiltredData = modelYDefault.filter((item) => {
      return item.AraclarName.toLowerCase().includes(search.toLowerCase())
    })
    setModelY(newFiltredData)
  }, [search])
  const columns = [
    {
      Header: "Araba Adı",
        ...
        return (
          <div className="btn-group">
            <div className='hstack gap-2 justify-content-end'>
              <Link to={"/admin/model-y-update/" + cellProps.row.original.AraclarID} className="fw-medium link-primary"><FeatherIcon size={12} color={'success'} icon="eye" /></Link>
                  ...
                  <div className="page-title-right">
                    <ol className="breadcrumb m-0">
                      <li className="breadcrumb-item">
                        <Link to={"/admin/model-y"} >Arabalar</Link>
                      </li>
                      <li className="breadcrumb-item active">Model-y</li>
                    </ol>
                  </div>
                </div>
              </CardBody>
                  ...
                  <div className="card-header border-0">
                    <div className="row g-4">
                      ...
                      <div className="col-sm-auto">
                        <div>
                          <Link
                            to="/admin/model-y-add/"
                            className="btn btn-success"
                          >
                            <i className="ri-add-line align-bottom me-1"></i> Model-y Ekle
                          </Link>
                        </div>
                      </div>
