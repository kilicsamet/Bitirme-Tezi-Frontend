...
const Index = () => {
  ...
  const handlePageChange = async () => {
    ...
    response.data.map((item)=>{
      if (item.AraclarModel==="Model S") {
    setModelS(modelS=>[...modelS,item])
    setModelSDefault(modelSDefault=>[...modelSDefault,item])
      }
    })
  }
  useEffect(() => {
  ...
  useEffect(() => {
    const newFiltredData = modelSDefault.filter((item) => {
      return item.AraclarName.toLowerCase().includes(search.toLowerCase())
    })
    setModelS(newFiltredData)
  }, [search])
  const columns = [
    {
      Header: "Araba Adı",
        ...
        return (
          <div className="btn-group">
            <div className='hstack gap-2 justify-content-end'>
              <Link to={"/admin/model-s-update/" + cellProps.row.original.AraclarID} className="fw-medium link-primary"><FeatherIcon size={12} color={'success'} icon="eye" /></Link>
                  ...
                  <div className="page-title-right">
                    <ol className="breadcrumb m-0">
                      <li className="breadcrumb-item">
                        <Link to={"/admin/model-s"} >Arabalar</Link>
                      </li>
                      <li className="breadcrumb-item active">Model-s</li>
                    </ol>
                  </div>
                </div>
              </CardBody>
                  ...
                  <div className="card-header border-0">
                    <div className="row g-4">
                      ...
                      <div className="col-sm-auto">
                        <div>
                          <Link
                            to="/admin/model-s-add/"
                            className="btn btn-success"
                          >
                            <i className="ri-add-line align-bottom me-1"></i> Model-s Ekle
                          </Link>
                        </div>
                      </div>
