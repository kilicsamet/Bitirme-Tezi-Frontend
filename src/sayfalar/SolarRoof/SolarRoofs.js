...
const Index = () => {
  const [solarRoof, setSolarRoof] = useState([]);
  const [deleteMethod, setDeleteMethod] = useState(false);
  const [solarRoofDefault, setSolarRoofDefault] = useState([]);
  ...
  const handlePageChange = async () => {
    ...
    response.data.map((item)=>{
      if (item.UrunlerName==="Solar Roof") {
    setSolarRoof(solarRoof=>[...solarRoof,item])
    setSolarRoofDefault(solarRoofDefault=>[...solarRoofDefault,item])
      }
    })
  }
  useEffect(() => {
  ...
  useEffect(() => {
    const newFiltredData = solarRoofDefault.filter((item) => {
      return item.UrunlerName.toLowerCase().includes(search.toLowerCase())
    })
    setSolarRoof(newFiltredData)
  }, [search])
  const columns = [
    {
      Header: "Ürün Adı",
        ...
        return (
          <div className="btn-group">
            <div className='hstack gap-2 justify-content-end'>
              <Link to={"/admin/solar-roof-update/" + cellProps.row.original.UrunlerID} className="fw-medium link-primary"><FeatherIcon size={12} color={'success'} icon="eye" /></Link>
                ...
                <div className="page-title-box d-sm-flex align-items-center justify-content-between">
                  ...
                  <div className="page-title-right">
                    <ol className="breadcrumb m-0">
                      ...
                      <li className="breadcrumb-item active">Solar Roof</li>
                    </ol>
                  </div>
                </div>
              </CardBody>
                  ...
                  <div className="card-header border-0">
                    <div className="row g-4">
                      ...
                      <div className="col-sm-auto">
                        <div>
                          <Link
                            to="/admin/solar-roof-add/"
                            className="btn btn-success"
                          >
                            <i className="ri-add-line align-bottom me-1"></i> Solar Roof Ekle
                          </Link>
                        </div>
                      </div>
